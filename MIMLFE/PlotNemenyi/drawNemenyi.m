...
% Results数据时各算法在各数据中的排位数据
% Names为算法名称
figure(1)
cd = CalculateCD(Results);
Ne = size(Results,2);
...
% 在这里可以根据自己的审美更改线的大小，颜色
for i = 1:divInd
    
    plot([RanSort(i) RanSort(i)],[-0.00 -0.00-(i*LinSpa)],'r')
    plot([1 RanSort(i)],[-0.00-(i*LinSpa) -0.00-(i*LinSpa)],'r')
...
for c = length(RanSort):-1:divInd+1
    ...
    plot([RanSort(c) RanSort(c)],[-0.00 -0.00-(i*LinSpa)],'r')
    plot([RanSort(c) Ne],[-0.00-(i*LinSpa) -0.00-(i*LinSpa)],'r')
